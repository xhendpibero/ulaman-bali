...
I’d group endpoints by resource (`/api/rooms`, `/api/facilities`, `/api/pages/:slug`) and return `{ data, meta }` payloads using Laravel API Resources to keep formatting consistent. Authentication would use Sanctum so the front end can remain stateless while still protecting admin-only routes. Versioning via `/api/v1` gives us room to evolve without breaking the front end.
